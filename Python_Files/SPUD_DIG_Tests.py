...
class DTA_MALI():
    def __init__(self,
        ...
        '''
        knn (int): Number of nearest neighbors to build the graphs
        t (int): Number of diffusion steps, not use if distances = "DPT"
        lamb: labels vs correspondece distance balance
        npca: Number of pca components before grpah construction 
        distances (str): "DPT" for diffusion pseudotime or "Diff"
        entR (float): entropy regularization for optimal transport 
        m (float) [0,1]: amount of mass to be transported 
        distance: distance to be passed to cdist to compute between diffusion operators 
        '''
    ...
    def compute_graphs(self):
        ...
        self.graphD1 = graphtools.Graph(self.domain1,
                                        ...
                                        decay=self.decay,
                                        thresh=1e-4,
                                        verbose=self.verbose) #**(self.kwargs); I took this line out, as their is no implementation to set this
    ...
    def compute_labels_distance(self):
        '''
        compute distances using labels only 
        '''
    ...
    def compute_corres_distance(self): 
        ...
        '''
        compute distance using one-to-one correspondences 
        '''
    ...
    def fit(self, domain1, domain2, sharedD1 = None, sharedD2 = None, 
        ...
        T = self.T/self.T.max() #T is like their projection matrix
        indx1, indx2 = np.nonzero(self.T)
        
        Tc = np.zeros((self.N1+self.Nshared, self.N2+self.Nshared))
        Tc[:self.N1, :self.N2] = T #Sets the values to T
        Tc[self.N1:, self.N2:] = np.eye((self.Nshared)) #Puts ones in the extra shared space
